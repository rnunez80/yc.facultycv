...
from yc.facultycv import DoDepartmentVote
from yc.facultycv import ViewDepartmentVote
from yc.facultycv.interfaces import IDepartmentVote
from yc.facultycv.viewlets.department import Vote as Department
...
class Vote(Service):
    ...
    def reply(self):
        ...
        can_vote = api.user.has_permission(DoDepartmentVote, obj=self.context)
        if not can_vote:
...
class Votes(Service):
    ...
    def reply(self):
        can_view_votes = api.user.has_permission(ViewDepartmentVote, obj=self.context)
...
def vote_info(obj, request=None):
    ...
    if not request:
        request = getRequest()
    voting = IDepartmentVote(obj)
    can_vote = api.user.has_permission(DoDepartmentVote, obj=obj)
